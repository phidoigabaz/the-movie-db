...
open class CardSliderViewController: UIViewController, UIScrollViewDelegate {
	...
	override open func viewDidLoad() {
        ...
        collectionView.transform = CGAffineTransform(rotationAngle: .pi / -2 * -90)
	}
    ...
    @IBAction func onSearchPressed(_ sender: Any) {
        SearchViewModel.pushActionSearch(self)
    }
...
extension CardSliderViewController: UICollectionViewDelegate, UICollectionViewDataSource {
	...
	public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cellID, for: indexPath) as! CardSliderCell
        let item = dataSource.item(for: dataSource.numberOfItems() - indexPath.item - 1)
        if let url = URL(string: item.image) {
            cell.imageView.transform = CGAffineTransform(rotationAngle: .pi / -2 * -90)
...
//        }
	...
	public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        MovieDetailViewModel.presentMovieDetailModule(item.id, navigation: self)
	}
}
...
extension CardSliderViewController: CardsLayoutDelegate {
	func transition(between currentIndex: Int, and nextIndex: Int, progress: CGFloat) {
		...
		ratingView.rating = (progress > 0.5 ? nextItem : currentItem).rating!/2
